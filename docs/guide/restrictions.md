...
5. **Correctness model**
    ...
    - $2^{64} \le n$ in cubic budget (cube root $\le 3\cdot10^{7}$, $4kn$ fits in 128 bits): same n−1 then cubic ladder (CLI default). Else $\lfloor\sqrt{n}\rfloor \le 2.5\cdot10^{10}$ (≤128-bit): full trial via OpenMP **u128** core or stdlib wheel
