...
5. **Correctness model**
    ...
    - still larger $n$: **BLS only** below 256 bits (`DEFAULT_N` is this band, `u128_nm1`). **FastECPP only** for $256\le$ bits $\le 40\,000$ (class-number-1 is inside that walk). A miss raises `UnsettledPrimalityError`. AKS is not a product-path fallback.
