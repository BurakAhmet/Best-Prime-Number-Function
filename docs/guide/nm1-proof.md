...
When $n-1$ is smooth (e.g. $600000000000000000001$, where $n-1=2^{21}\cdot 3\cdot 5^{20}$), step 1 is microseconds and step 2 is a handful of modular exponentiations. The current CLI default $10000000000000000000000013$ has a hostile $n-1$ and falls through to cubic. Same machine class:
...
| CLI default (84-bit, hostile $n-1$) | — | inconclusive → **cubic ~0.3 s** |
