...
When $n-1$ is smooth (e.g. $600000000000000000001$, where $n-1=2^{21}\cdot 3\cdot 5^{20}$), step 1 is microseconds and step 2 is a handful of modular exponentiations. The current CLI default $100000000000000000000000000000000000000000031$ (147-bit) settles via **partial Pocklington** after trial/Brent/p−1 splits of $n-1$ — even past the C cubic wall ($4kn$ no longer fits in 128 bits). Same machine class:
...
| CLI default (147-bit) | cubic incomplete ($4kn>128$ bits) | **~0.1–0.3 s** n−1 |
