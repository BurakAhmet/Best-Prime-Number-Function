...
5. **Correctness model**
   ...
   - still larger $n$: **BLS only** below 256 bits (`DEFAULT_N` is this band, `u128_nm1`). **FastECPP** for $256\le$ bits $<800$ (class-number-1 is inside that walk). **Cyclotomic APR-CL** from 800 bits while its modulus exceeds $\sqrt{n}$, else FastECPP through $40\,000$ bits. A miss raises `UnsettledPrimalityError`. AKS is not in the library.
