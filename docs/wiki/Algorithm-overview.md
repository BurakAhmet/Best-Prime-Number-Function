...
- Still larger: **BLS only** when $n$ has fewer than 256 bits (147-bit CLI default). **FastECPP** when $n$ has $\ge 256$ bits (class-number-1 lives inside that walk), then **cyclotomic APR-CL** from 800 bits. A miss raises `UnsettledPrimalityError`. AKS is not in the library.
