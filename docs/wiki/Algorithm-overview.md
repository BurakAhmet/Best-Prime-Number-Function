...
2. If `is_prime_data/wheel_core.so` is present: **OpenMP C** (preferred on Linux CI), with:
   ...
   - **combined BLS** then complete cubic search when $\lfloor\sqrt{n}\rfloor \ge 10^{7}$ and `lehman_factor_u128` is present (`u64_nm1` / `u64_lehman_c`),
...
1. **Combined BLS** first for every multi-limb hard $n$: n−1 Pocklington / Theorem 5, Lucas n+1 ($G>\sqrt{n}$ or complete), Combined Theorem 1 ($n < \max(F^{2}G/2,\,FG^{2}/2)$, **not** $FG>\sqrt{n}$). The **147-bit CLI default** still settles as `u128_nm1` — even when $4kn$ no longer fits in 128 bits (cubic C incomplete). `DEFAULT_N` is unchanged.
...
4. For still-larger $n$: **30030-wheel** trial up to $\min(10^8,\lfloor\sqrt{n}\rfloor)$, Fermat / ECM / SIQS split of $n$, then **deterministic Atkin–Morain ECPP** (class-number-1, then small-$h$ CM with $h(D)\le 16$). **General 100-digit completeness is the small-$h$ layer**; $h=1$ is a curve-construction convenience, not a random-100-digit engine. **AKS** is the last resort (Kronecker poly mul; correct, still slow for huge primes). Paths `bigint_bls` / `bigint_ecpp` / `bigint_trial_or_aks`.
...
| `lehman_factor` | Complete $O(n^{1/3})$ split; hard-path fallback after combined BLS |
