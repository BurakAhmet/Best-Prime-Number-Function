...
CLI after install: `is-prime`, `next-prime`, `next-primes`, `prime-count`, `primality-certificate`, … Exit 0 = prime, 1 = not prime, 2 = bad input. Default `is-prime` yardstick is `600000000000000000001` (70-bit; n−1 Pocklington when $n-1$ factors, else OpenMP cubic). Printed `TIME` is **end-to-end** (import + check).
