...
| Near $2^{63}$ prime | 9223372036854775783 | ~0.16–0.20 s |
| Largest prime $<2^{64}$ | 18446744073709551557 | ~0.23–0.28 s |
| Mersenne M61 | $2^{61}-1$ | ~0.09–0.11 s |
...
| [**Optimize**](.github/workflows/optimize.yml) | daily 05:00 UTC / manual | Baseline; catalog hunt; assign **Copilot** to a new `[Optimize]` issue; examine PRs; merge only if faster |
...
**Policy:** same-repo PRs may be auto-approved and auto-merged after green **CI** + **Determinism**; **forks are never** auto-approved or auto-merged. PRs labeled `optimize/candidate` skip generic Auto-merge — Optimize examines them and merges only when they are faster than `main`. Auto-assigning Copilot needs repo secret `COPILOT_ASSIGN_TOKEN`.
