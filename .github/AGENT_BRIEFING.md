...
**CI gates:** build `wheel_core.so`, restriction linter, wiki sync, pytest not slow, Linux `lab(10**9+7)["path"]=="u64_wheel_c"` and hard 64-bit `u64_lehman_c`, **e2e** perf vs base, Determinism.  
**Paths:** `u64_wheel_c` / `u64_lehman_c` / `u128_lehman_c` / `u128_wheel_c` (OpenMP), `python_wheel`, `bigint_wheel`, `bigint_trial_or_aks`.  
